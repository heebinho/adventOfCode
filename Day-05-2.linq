...
var row = seats
	.Select(s => new { row = s.Substring(0, 7), col = s.Substring(7) })
	.Select(s => new {
		RowNo = which(Enumerable.Range(0, 128), s.row, 0),
		ColNo = which(Enumerable.Range(0, 8), s.col, 0)
	})
	.OrderBy(o=>o.RowNo).ThenBy(o=>o.ColNo)
	.GroupBy(s=>s.RowNo)
	.Where(g => g.Sum(a => a.ColNo) != 28)
	.Select(s => new { Row=s, Sum=s.Sum(a => a.ColNo)})
	.Skip(1).First()
	.Dump();

calcSeatId(row.Row.Key, 28- row.Sum).Dump();
